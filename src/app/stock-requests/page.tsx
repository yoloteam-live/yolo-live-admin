...
import {
  Search, Loader2, CheckCircle2, XCircle, RefreshCw, Diamond, Package,
} from 'lucide-react';
...
export default function StockRequestsPage() {
  ...
  useEffect(() => {
    const ch = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'agency_stock_requests' }, fetchAll)
      .subscribe();
  ...
  async function fetchAll() {
    ...
    let q = supabase
      ...
      .select(
        `*,
         agency:agencies!agency_stock_requests_agency_id_fkey(name, code),
         requester:profiles!agency_stock_requests_requested_by_fkey(full_name, display_id, phone_number),
         fulfiller:profiles!agency_stock_requests_fulfilled_by_fkey(full_name)`
      )
      .order('created_at', { ascending: false })
      .limit(100);
    if (statusFilter !== 'all') q = q.eq('status', statusFilter);
    const { data, error } = await q;
    if (error) console.error('Stock fetch error:', error);
    else setRequests((data || []) as any);
    setLoading(false);
  }

  async function fulfill(id: string, agencyName: string, amount: number) {
    if (!currentAdminId) return;
    const ok = window.confirm(
      `Fulfill ${amount.toLocaleString()} diamonds to ${agencyName}?\n\nMake sure you have received their payment first.`
    );
    if (!ok) return;
    ...
    alert(`Fulfilled. ${amount.toLocaleString()} diamonds added to ${agencyName}.`);
    fetchAll();
  }

  async function markContacted(id: string) {
    setActionLoading(id);
    const { error } = await supabase.from('agency_stock_requests').update({ status: 'contacted' }).eq('id', id);
    setActionLoading(null);
  ...
  async function cancel(id: string) {
    ...
    if (!ok) return;
    setActionLoading(id);
    const { error } = await supabase.from('agency_stock_requests').update({ status: 'cancelled' }).eq('id', id);
    setActionLoading(null);
    if (error) alert('Error: ' + error.message);
    else fetchAll();
  }
  ...
  const filtered = requests.filter((r) => {
    ...
    return (
      r.agency?.name?.toLowerCase().includes(s) ||
      r.agency?.code?.toLowerCase().includes(s) ||
      ...
      <div className="flex justify-between items-center flex-wrap gap-4">
        <div>
          <div className="flex items-center gap-3">
            <h2 className="text-3xl font-black text-white">Stock Requests</h2>
            {pendingCount > 0 && (
          ...
          <p className="text-gray-500 mt-1">Agency owners requesting bulk diamonds from the master supply.</p>
        </div>
        <div className="flex gap-3 flex-wrap">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search agency, owner…"
        ...
        <table className="w-full text-left">
          ...
          <tbody className="text-sm">
            {loading && requests.length === 0 ? (
              ...
              <tr><td colSpan={6} className="px-6 py-12 text-center text-gray-500">No stock requests.</td></tr>
            ) : filtered.map((r) => (
              <tr key={r.id} className="border-b border-white/5 hover:bg-white/[0.02] transition-all">
                ...
                <td className="px-6 py-4">
                  ...
                  <p className="text-[10px] text-gray-500 font-mono">{r.agency?.code}</p>
                </td>
                <td className="px-6 py-4">
                ...
                <td className="px-6 py-4 text-right">
                  {r.status === 'pending' || r.status === 'contacted' ? (
                    <div className="flex justify-end gap-2">
                      ...
                      <button
                        onClick={() => fulfill(r.id, r.agency?.name || 'Agency', r.diamond_amount)}
                        disabled={actionLoading === r.id}
