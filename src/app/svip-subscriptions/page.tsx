...
type SvipSubscription = {
  ...
  intro2_name: string;
  intro2_thumbnail_url: string;
  intro2_video_url: string;
  frame_name: string;
  frame_url: string;
  frame2_name: string;
  frame2_url: string;
...
const emptySubscription: SvipSubscription = {
  id: '',
  name: '',
  price: 0,
  duration_days: 30,
...
export default function SvipSubscriptionsPage() {
  const router = useRouter();
  const { isSuperAdmin, loading: roleLoading } = useAdminRole();
  const [rows, setRows] = useState<SvipSubscription[]>([]);
  const [loading, setLoading] = useState(true);
  const [editing, setEditing] = useState<SvipSubscription | null>(null);
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const { data, error } = await supabase.from('svip_subscriptions').select('*')
      .order('display_order').order('created_at');
    if (error) alert(`Could not load SVIP subscriptions: ${error.message}`);
    setRows(((data as SvipSubscription[]) || []).map((row) => ({
      ...row,
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('admin-svip-subscriptions')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'svip_subscriptions' }, () => load())
      .subscribe();
    return () => { supabase.removeChannel(channel); };
  }, [isSuperAdmin, load]);

  async function remove(row: SvipSubscription) {
    if (!window.confirm(`Delete "${row.name}"?`)) return;
    const { error } = await supabase.from('svip_subscriptions').delete().eq('id', row.id);
    ...
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between gap-4 mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-black text-white">SVIP Subscriptions</h1>
            <p className="text-xs text-gray-500">SVIP-only packages with subscription, two frames, two intros and name tag</p>
          </div>
        </div>
        <button
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-[#2b2750] bg-[#17152e] p-14 text-center text-gray-500">
          No SVIP subscriptions yet.
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
          {rows.map((row) => (
            <div key={row.id} className="rounded-2xl overflow-hidden border border-[#302b55] bg-[#17152e]">
              <div className="p-4" style={{ background: `linear-gradient(135deg, ${row.accent_color || '#F59E0B'}33, transparent)` }}>
              ...
              <div className="p-4 grid grid-cols-2 gap-3">
                <div className="rounded-xl border border-white/5 bg-[#0e1029] p-3">
                  <div className="text-[10px] uppercase font-bold text-gray-500 mb-2">Frame 1</div>
                  ...
                  <div className="text-white text-xs font-bold truncate mt-2">{row.frame_name || 'SVIP Frame'}</div>
                </div>
                <div className="rounded-xl border border-white/5 bg-[#0e1029] p-3">
                  <div className="text-[10px] uppercase font-bold text-gray-500 mb-2">Frame 2</div>
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={previewUrl(row.frame2_url, BUNDLED_FRAMES)} alt={row.frame2_name} className="w-full aspect-square object-contain" />
                  <div className="text-white text-xs font-bold truncate mt-2">{row.frame2_name || 'SVIP Frame'}</div>
                </div>
                <div className="rounded-xl border border-white/5 bg-[#0e1029] p-3">
                  <div className="text-[10px] uppercase font-bold text-gray-500 mb-2">Intro 1</div>
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  ...
                  <div className="text-white text-xs font-bold truncate mt-2">{row.intro_name || 'SVIP Intro'}</div>
                </div>
                <div className="rounded-xl border border-white/5 bg-[#0e1029] p-3">
                  <div className="text-[10px] uppercase font-bold text-gray-500 mb-2">Intro 2</div>
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={previewUrl(row.intro2_thumbnail_url, BUNDLED_THUMBS)} alt={row.intro2_name} className="w-full aspect-square object-cover rounded-lg" />
                  <div className="text-white text-xs font-bold truncate mt-2">{row.intro2_name || 'SVIP Intro'}</div>
                </div>
              </div>
...
function SvipSubscriptionModal({ row, creating, onClose }: { row: SvipSubscription; creating: boolean; onClose: () => void }) {
  ...
  const frame2Ref = useRef<HTMLInputElement>(null);

  function buildPath(file: File, kind: 'thumb' | 'video' | 'thumb2' | 'video2' | 'frame' | 'frame2') {
    const base = slugify(value.id || value.name || 'svip-package') || 'svip-package';
    const ext = (file.name.split('.').pop() || (kind.startsWith('video') ? 'mp4' : 'webp')).toLowerCase();
    return `${base}-${kind}-${Date.now()}.${ext}`;
  }

  async function uploadThumb(file: File, slot: 1 | 2 = 1) {
    if (!['image/jpeg', 'image/png', 'image/webp'].includes(file.type)) {
      alert('Thumbnail must be JPG, PNG, or WebP.');
      return;
    }
    setUploading(slot === 1 ? 'thumb' : 'thumb2');
    ...
    const path = buildPath(uploadFile, slot === 1 ? 'thumb' : 'thumb2');
    ...
    setValue((current) => (slot === 1
      ? { ...current, intro_thumbnail_url: url }
      : { ...current, intro2_thumbnail_url: url }));
  }

  async function uploadIntroVideo(file: File, slot: 1 | 2 = 1) {
    ...
    setUploading(slot === 1 ? 'video' : 'video2');
    const path = buildPath(file, slot === 1 ? 'video' : 'video2');
    ...
    const url = supabase.storage.from(INTRO_BUCKET).getPublicUrl(path).data.publicUrl;
    setValue((current) => (slot === 1
      ? { ...current, intro_video_url: url }
      : { ...current, intro2_video_url: url }));
  }

  async function uploadFrame(file: File, slot: 1 | 2 = 1) {
    ...
    setUploading(slot === 1 ? 'frame' : 'frame2');
    const path = buildPath(file, slot === 1 ? 'frame' : 'frame2');
    ...
    setValue((current) => (slot === 1
      ? { ...current, frame_url: url }
      : { ...current, frame2_url: url }));
  }
  ...
  async function save() {
    ...
    if (!id || !value.name.trim() || !value.intro_thumbnail_url || !value.intro_video_url || !value.intro2_thumbnail_url || !value.intro2_video_url || !value.frame_url || !value.frame2_url) {
      alert('ID, name, two intro thumbnails, two intro videos, and two frames are required.');
      return;
    }
    ...
    const payload = {
      ...
      intro2_name: value.intro2_name.trim(),
      intro2_thumbnail_url: value.intro2_thumbnail_url.trim(),
      intro2_video_url: value.intro2_video_url.trim(),
      frame_name: value.frame_name.trim(),
      frame_url: value.frame_url.trim(),
      frame2_name: value.frame2_name.trim(),
      frame2_url: value.frame2_url.trim(),
    ...
    const { error } = creating
      ? await supabase.from('svip_subscriptions').insert(payload)
      : await supabase.from('svip_subscriptions').update(payload).eq('id', row.id);
    setSaving(false);
    ...
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="w-full max-w-5xl max-h-[92vh] overflow-y-auto rounded-3xl border border-[#39325f] bg-[#18162f]">
        <div className="sticky top-0 bg-[#18162f] z-10 p-5 border-b border-white/5 flex justify-between items-center">
          <h2 className="text-lg font-black text-white">{creating ? 'Add SVIP Package' : `Edit ${row.name}`}</h2>
        ...
        <div className="p-5 grid lg:grid-cols-[1.1fr_.9fr] gap-5">
          <div className="space-y-4">
            <div className="grid md:grid-cols-2 gap-3">
              <label className="block text-xs text-gray-400">Package ID
                <input className={`${input} mt-1`} disabled={!creating} value={value.id} onChange={(e) => setValue({ ...value, id: e.target.value })} placeholder="svip-royal-duo" />
              </label>
            ...
            <div className="grid md:grid-cols-2 gap-3">
              <label className="block text-xs text-gray-400">Intro 1 name
                <input className={`${input} mt-1`} value={value.intro_name} onChange={(e) => setValue({ ...value, intro_name: e.target.value })} />
              </label>
              <label className="block text-xs text-gray-400">Intro 2 name
                <input className={`${input} mt-1`} value={value.intro2_name} onChange={(e) => setValue({ ...value, intro2_name: e.target.value })} />
              </label>
              <label className="block text-xs text-gray-400">Frame 1 name
                <input className={`${input} mt-1`} value={value.frame_name} onChange={(e) => setValue({ ...value, frame_name: e.target.value })} />
              </label>
              <label className="block text-xs text-gray-400">Frame 2 name
                <input className={`${input} mt-1`} value={value.frame2_name} onChange={(e) => setValue({ ...value, frame2_name: e.target.value })} />
              </label>
            </div>

            <label className="block text-xs text-gray-400">Intro 1 thumbnail URL
              <input className={`${input} mt-1`} value={value.intro_thumbnail_url} onChange={(e) => setValue({ ...value, intro_thumbnail_url: e.target.value })} />
            </label>
            <label className="block text-xs text-gray-400">Intro 1 video URL
              <input className={`${input} mt-1`} value={value.intro_video_url} onChange={(e) => setValue({ ...value, intro_video_url: e.target.value })} />
            </label>
            <label className="block text-xs text-gray-400">Intro 2 thumbnail URL
              <input className={`${input} mt-1`} value={value.intro2_thumbnail_url} onChange={(e) => setValue({ ...value, intro2_thumbnail_url: e.target.value })} />
            </label>
            <label className="block text-xs text-gray-400">Intro 2 video URL
              <input className={`${input} mt-1`} value={value.intro2_video_url} onChange={(e) => setValue({ ...value, intro2_video_url: e.target.value })} />
            </label>
            <label className="block text-xs text-gray-400">Frame 1 URL
              <input className={`${input} mt-1`} value={value.frame_url} onChange={(e) => setValue({ ...value, frame_url: e.target.value })} />
            </label>
            <label className="block text-xs text-gray-400">Frame 2 URL
              <input className={`${input} mt-1`} value={value.frame2_url} onChange={(e) => setValue({ ...value, frame2_url: e.target.value })} />
            </label>
          </div>
          ...
          <div className="space-y-3">
            <button onClick={() => thumbRef.current?.click()} disabled={busy} className="w-full rounded-2xl border border-dashed border-cyan-500/50 bg-[#0e1029] p-3 text-cyan-100 flex items-center justify-center gap-2 disabled:opacity-50">
              ...
              Upload intro 1 thumbnail
            </button>
            <input ref={thumbRef} hidden type="file" accept=".jpg,.jpeg,.png,.webp,image/*" onChange={(e) => e.target.files?.[0] && uploadThumb(e.target.files[0], 1)} />
            ...
            <button onClick={() => videoRef.current?.click()} disabled={busy} className="w-full rounded-2xl border border-dashed border-violet-500/50 bg-[#0e1029] p-3 text-violet-100 flex items-center justify-center gap-2 disabled:opacity-50">
              ...
              Upload intro 1 video
            </button>
            <input ref={videoRef} hidden type="file" accept=".mp4,video/mp4" onChange={(e) => e.target.files?.[0] && uploadIntroVideo(e.target.files[0], 1)} />

            <button onClick={() => thumb2Ref.current?.click()} disabled={busy} className="w-full rounded-2xl border border-dashed border-cyan-500/50 bg-[#0e1029] p-3 text-cyan-100 flex items-center justify-center gap-2 disabled:opacity-50">
              {uploading === 'thumb2' ? <Loader2 className="animate-spin" size={16} /> : <Upload size={16} />}
              Upload intro 2 thumbnail
            </button>
            <input ref={thumb2Ref} hidden type="file" accept=".jpg,.jpeg,.png,.webp,image/*" onChange={(e) => e.target.files?.[0] && uploadThumb(e.target.files[0], 2)} />

            <button onClick={() => video2Ref.current?.click()} disabled={busy} className="w-full rounded-2xl border border-dashed border-violet-500/50 bg-[#0e1029] p-3 text-violet-100 flex items-center justify-center gap-2 disabled:opacity-50">
              {uploading === 'video2' ? <Loader2 className="animate-spin" size={16} /> : <Video size={16} />}
              Upload intro 2 video
            </button>
            <input ref={video2Ref} hidden type="file" accept=".mp4,video/mp4" onChange={(e) => e.target.files?.[0] && uploadIntroVideo(e.target.files[0], 2)} />
            ...
            <input ref={frameRef} hidden type="file" accept=".webp,image/webp" onChange={(e) => e.target.files?.[0] && uploadFrame(e.target.files[0], 1)} />

            <button onClick={() => frame2Ref.current?.click()} disabled={busy} className="w-full rounded-2xl border border-dashed border-pink-500/50 bg-[#0e1029] p-3 text-pink-100 flex items-center justify-center gap-2 disabled:opacity-50">
              {uploading === 'frame2' ? <Loader2 className="animate-spin" size={16} /> : <ImageIcon size={16} />}
              Upload frame 2 WebP
            </button>
            <input ref={frame2Ref} hidden type="file" accept=".webp,image/webp" onChange={(e) => e.target.files?.[0] && uploadFrame(e.target.files[0], 2)} />

            <div className="grid grid-cols-2 gap-3 pt-2">
              <div className="rounded-2xl overflow-hidden border border-[#39325f] bg-[#0e1029] p-3">
                <div className="text-[10px] uppercase font-bold text-gray-500 mb-2">Frame 1 Preview</div>
                {value.frame_url ? (
                  // eslint-disable-next-line @next/next/no-img-element
              ...
              <div className="rounded-2xl overflow-hidden border border-[#39325f] bg-[#0e1029] p-3">
                <div className="text-[10px] uppercase font-bold text-gray-500 mb-2">Frame 2 Preview</div>
                {value.frame2_url ? (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={previewUrl(value.frame2_url, BUNDLED_FRAMES)} alt="Frame 2 preview" className="w-full aspect-square object-contain" />
                ) : (
                  <div className="w-full aspect-square flex items-center justify-center text-gray-600"><ImageIcon size={38} /></div>
                )}
              </div>
              <div className="rounded-2xl overflow-hidden border border-[#39325f] bg-[#0e1029] p-3">
                <div className="text-[10px] uppercase font-bold text-gray-500 mb-2">Intro 1 Preview</div>
              ...
              <div className="rounded-2xl overflow-hidden border border-[#39325f] bg-[#0e1029] p-3">
                <div className="text-[10px] uppercase font-bold text-gray-500 mb-2">Intro 2 Preview</div>
                {value.intro2_thumbnail_url ? (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={previewUrl(value.intro2_thumbnail_url, BUNDLED_THUMBS)} alt="Intro 2 preview" className="w-full aspect-square object-cover rounded-xl" />
                ) : (
                  <div className="w-full aspect-square flex items-center justify-center text-gray-600"><Video size={38} /></div>
                )}
              </div>
            ...
            <p className="text-[11px] leading-5 text-gray-500">
              SVIP package assets are stored here only. They are not inserted into the Mall Intro or Profile Frame tabs.
            </p>
          </div>
        </div>
